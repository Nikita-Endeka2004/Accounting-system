...
export const categoriesAction = async ({request}: any) => {
  switch(request.method){
    ...
    case "PATCH": {
      ...
      await instance.patch(`/categories/caregory/${category.id}`, category)
      return null
    }
    case "DELETE": {
      const formData = await request.formData()
      const categoryId = formData.get('id')
      await instance.delete(`/categories/caregory/${categoryId}`)
      return null
    }
  }
}
...
const Categories: FC = () => {
  ...
  return (
    <>
      ...
      <button onClick={() => setVisibleModel(true)} className='max-w-fit flex items-center gap-2 text-white/50 mt-5 hover:text-white'>
        <FaPlus/>
      ...
      {visibleModal && isEdit &&(<CategoryModel type='patch' id={categoryId} setVisibleModel={setVisibleModel}/>)}
